...
const MarkerPlayer = Marker.extend({
    ...
    _interpolatePosition(p1, p2, duration, t) {
        ...
        r = (r > 0) ? r : 0;
        r = (r > 1) ? 1 : r;
        return latLng(p1.lat + r * (p2.lat - p1.lat), p1.lng + r * (p2.lng - p1.lng));
    },
    _startAnimation() {
    ...
    _animate(timestamp, noRequestAnim) {
        ...
        if (elapsedTime != null) {
            ...
            let p = this._interpolatePosition(latLng(this._currentLine[0].latlng), latLng(this._currentLine[1].latlng), this._currentDuration, elapsedTime);
            this.setLatLng(p);
        }
        if (!noRequestAnim) {
            this._animRequested = true;
...
//# sourceMappingURL=markerplayer-src.esm.js.map
